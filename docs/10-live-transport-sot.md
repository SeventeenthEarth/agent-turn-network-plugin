...
- `plugin/NEWFIX-008` must repair or clarify live `command.submit` representation for structured `council.lock_agenda` payloads without introducing hidden CLI fallback, local lifecycle state, local dedupe, or generated daemon IDs. If the public `idempotency_key` cannot be represented as daemon `command_id`, the plugin must either fail closed with actionable operator guidance or use an explicitly approved daemon protocol representation.
