...
- `cross-repo/NEWFIX-009` must verify that plugin and control paths preserve `decision_question`, `success_criteria`, and `out_of_scope_policy` through `agenda_locked.payload` and selected-runner prompt evidence before reporting this defect repaired.
...
| 9 | cross-repo | NEWFIX-009 | planned | Cross-repo agenda-context acceptance proof after NEWFIX-007/008; must show required agenda fields in daemon agenda payload and selected-runner prompt evidence, with missing-field fail-closed diagnostics preserved. |
