...
| HUN-004 | plugin | local implementation proof | Renamed Python distribution/import package, plugin manifest, entrypoint imports, package metadata/build/version/mypy paths, and package-load smoke under bounded local proof; HUN-006/HUN-009/HUN-010/HUN-013 are tracked as separate downstream tasks. |
| HUN-006 | plugin | local implementation proof | Hermes tool schemas, handlers, smoke scripts, manifest declarations, tests, and contract docs were renamed from `kan_*` / `KAN_*` to `hun_*` / `HUN_*` with no legacy aliases, dual registration, or fallback acceptance of old tool names. Local verification passed `git diff --check`, plugin `make test`, and sibling control `make check-plugin-contract`; MAR required role coverage passed after recovery. Live readiness, Discord delivery, daemon startup/discovery, runtime activation, package publication, profile/provider/gateway/auth/token/model mutation, commit, push, HUN-009 bundled skill rename, HUN-010 public docs scrub, and HUN-013 guardrails were separate from HUN-006; HUN-014 final compatibility is now completed/local-compatibility-proof. HUN-010 is now tracked separately in this table. |
| HUN-008 | plugin | local implementation proof | Reconfirmed activation planner evidence fields under HUN names with explicit vanilla Hermes profile/plugin visibility requirements, separated readiness axes, and historical RUNFIX/control IDs preserved only as provenance/dependency labels. |
| HUN-009 | plugin | local implementation proof | Renamed packaged bundled skills to `hun-plugin`, `hun-moderator`, and `hun-participant` with no legacy `kan-*` packaged aliases, folded moderation hard rules into `hun-moderator`, and updated public-safe packaged operator guidance under plugin KAH run `run-20260622T155451Z-3f916be65144`. Verification, MAR role coverage, and second color review passed; HUN-013 guardrails, live readiness, package publication, and profile/provider/gateway/auth/token mutation remain out of scope, while HUN-014 final compatibility is now completed/local-compatibility-proof. |
| HUN-010 | plugin | local docs proof | Scrubbed plugin public docs, package metadata, README, docs map, and operator docs for HUN-only wording and public release suitability under plugin KAH run `run-20260622T175656Z-6f21f88cbc6b`; remaining KAN/control/KAB terms are historical/provenance/safety-boundary labels, not public aliases. HUN-013 guardrails, live readiness, package publication, and profile/provider/gateway/auth/token mutation remain out of scope, while HUN-014 final compatibility is now completed/local-compatibility-proof. |
| HUN-013 | plugin | local implementation proof | Added plugin forbidden-term guardrails and HUN-only plugin-load smoke checks. Guardrails scan public/package surfaces for stale legacy package/tool/skill/path/manifest/command/compatibility/live-private claims with path/token/line/reason exceptions for approved historical, provenance, and safety-boundary labels only. Plugin-load smoke proves HUN manifest/package metadata, HUN tool names, HUN bundled skills, zero hooks, zero commands, zero registered resources, fail-closed handlers, live-looking environment inertness, and negative legacy fixtures. Verification passed focused HUN-013 unit tests, `make docs-guardrails`, `make check-plugin-load-smoke`, `make test-prepare`, `make check-core-contract`, plugin `make test`, and `git diff --check`; cross-repo/HUN-014 is completed/local-compatibility-proof, while live readiness, package publication, and profile/provider/gateway/auth/token mutation remain out of scope. |
| HUN-014 | cross-repo | completed/local-compatibility-proof | Completed local compatibility proof under run `run-20260623T033911Z-267d9cd68d9c` closes final local/public compatibility across `hun-control` and `hun-plugin`: env override precedence, local checkout path wording, docs-map status, and conformance labels. Live readiness, Discord delivery, hosted repository rename, package publication, profile/provider/gateway/auth/token mutation, push, and production rollout remain out of scope. |
