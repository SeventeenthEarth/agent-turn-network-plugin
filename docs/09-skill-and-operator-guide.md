...
- NEXFIX content-plane exception: for long or live-visible councils, absent or `blocked` `selected_runner_prompt_evidence` before start is a `start_blocker` because it proves the selected-runner prompt-envelope contract is unavailable or inadequate before the operator intentionally starts a context-sensitive discussion. For `plugin/NEWFIX-006`, the prompt own-history proof must consume the `control/NEWFIX-004` extension of `selected_runner_prompt_evidence`, and Discord-origin `live_visible_thread` requests also require `selected_runner_timeout_evidence` proving `dispatch_timeout_sec=120` or an explicitly approved alternative before start-capable status. While `control/NEWFIX-004` and `control/NEWFIX-005` remain `implementation_complete/review_pending`, the planner may report prompt/timeout evidence provisionally but must not emit `ready_to_start`; those rows do not become start authority until control review closeout. After start, per-turn selected-runner invocation/linkage remains `runtime_evidence_pending`, and final prompt-quality proof remains `final_acceptance_unproven` until durable evidence is present. Missing-context participant diagnostics must remain compatible with the current control selected-runner producer contract: `type: "speech"` with `payload.speech`, optional diagnostic `claims[]`, empty or valid `stance_links[]`, `contribution_type`, `new_axis_reason`, and optional `evidence[]`.
