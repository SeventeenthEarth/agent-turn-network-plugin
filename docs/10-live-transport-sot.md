...
- `atn-moderator` guidance must require `selected_runner_prompt_evidence` review before long or live-visible councils and must stop the run if a first speech says agenda/prior context is missing. Until `control/NEWFIX-004` is implemented, `selected_runner_prompt_evidence` is the named handoff from `control/NEWFIX-001`; acceptable evidence is a status/export field, deterministic fixture artifact, or equivalent non-secret prompt-capture proof with `session_id`, `speaker_selected_event_id`, `selected_member`, `turn`, agenda/prior-context source refs, `included_context[]`, `missing_required_context[]`, `prompt_context_sha256`, redacted prompt excerpt or fixture id, and `result`. After `control/NEWFIX-004`, plugin guidance must also expect distinguishable selected-member own-history source evidence.
...
- Plugin guidance consumes daemon projection authority from `control/NEWFIX-001` and its planned `NEWFIX-004`/`NEWFIX-005` extensions; plugin-side hints are optional diagnostics only and cannot replace control projection, control timeout evidence, or selected-runner delivery proof.
...
| 4 | control | NEWFIX-004 | planned | Control-owned selected-member own-history prompt context and prompt evidence source separation. Plugin consumes the resulting evidence only after the control contract is stable. |
| 5 | control | NEWFIX-005 | planned | Control-owned live-visible selected-runner timeout policy/evidence requiring `dispatch_timeout_sec=120` or approved alternative for Discord live-visible councils. Plugin consumes the resulting evidence only after the control contract is stable. |
| 6 | plugin | NEWFIX-006 | planned | Plugin-owned activation planner/operator guardrail consuming `NEWFIX-004` and `NEWFIX-005`: require timeout evidence and own-history prompt evidence for start-capable/live-visible reports, and keep local-only/artifact/manual bridge paths non-equivalent. |
