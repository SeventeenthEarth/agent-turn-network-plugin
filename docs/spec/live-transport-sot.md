...
PRSLR-008 through PRSLR-011 are the dialogue-quality extension. `control/PRSLR-008` has completed bounded local proof and approved local commit for selected-runner invocation and canonical speech linkage: fallback/manual profile text, runnerless speech, wrong-turn speech, and missing-selected-turn speech cannot satisfy `selected_runner_pass`; control now requires runner invocation success plus linked canonical speech for the same selected member and positive selected turn. `control/PRSLR-009` must define the `prior_speaker_argue_quality_evidence` contract and diagnostics: every non-opening quality-required speech targets at least one previous speaker/claim through valid `stance_links[]`; legacy `responds_to_event_id`, prose mentions, Discord order, and moderator summaries remain non-authoritative. `plugin/PRSLR-010` updates plugin schemas, local validation where caller context is sufficient, bundled guidance, and operator examples to produce those links without claiming daemon quality state. `cross-repo/PRSLR-011` proves the E2E dialogue-quality path and final docs closeout while preserving the same no-live/no-production/no-mutation boundaries.
