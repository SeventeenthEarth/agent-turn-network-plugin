...
| RUNFIX-012 | Activation planner consumes participant runtime readiness | planned | Consume `control/RUNFIX-011` readiness/status evidence in the pure/local activation planner, operator guide, bundled skill, and schemas so live-visible councils require explicit participant runtime subscriber/cursor/heartbeat, attendance handling, selected-runner readiness, and visible-surface proof. Gateway liveness, transcript/export artifacts, fallback/manual profile text, or parent-channel fallback alone must keep `live_readiness=false`. This task includes plugin RUNFIX docs/status reconciliation and does not mutate live profiles, providers, gateways, auth, tokens, Discord state, or production daemons without separate explicit approval. |
| RUNFIX-013 | KAN council moderation skill hard rules | planned | Add or strengthen packaged `kan-council-moderation` guidance from 주유's 2026-06-19 moderation-skill-gap report: no predeclared full speaker order for live councils; complete `council.new` → `request-attendance` → terminal attendance → `lock-agenda` → `prepare` → `ready`/`prepared_partial` first; each turn uses poll/hand-raise evaluation and a justified `speaker_selected`; `relevance` is the default mode; Discord replies are not state without daemon `speech`; moderator substantive opinions use participant-style turns; mistaken fixed-order flows cancel/restart before speech or repair-forward after speech. |
