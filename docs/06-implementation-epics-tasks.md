...
| LTRAN-003 | Prove plugin/CLI/daemon equivalence | planned | Run disposable data-home and disposable Hermes/plugin evidence showing CLI status/version/stream/write commands and plugin status/stream/participant-write tools address the same daemon with matching event/idempotency behavior. Stream/write support, equivalence proof, dedupe proof, and command-id/idempotency proof remain deferred to LTRAN-003. |
