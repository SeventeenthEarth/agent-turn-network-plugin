...
PRSLR is the eleven-task cross-repo epic for the persistent participant runtime SOT and the follow-on dialogue-quality hardening planned from the 2026-07-05 KLM token/speed final report (`17thHermes:40_outputs/team/jooyoo/atn/klm-token-speed-20260705T174553Z/final_report.md`). The plugin side is a consumer/adapter surface only. Control owns participant-session handles, generations, response windows, cursor/ack authority, rehydrate snapshots, fail-close events, selected-runner accounting, discussion-quality diagnostics, and the named handoffs `persistent_participant_runtime_evidence` plus `prior_speaker_argue_quality_evidence` introduced by PRSLR-009.
