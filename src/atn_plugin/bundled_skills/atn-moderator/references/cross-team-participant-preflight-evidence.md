...
Use this when 주군 asks non-ATN lane members, such as KLM 장수 profiles, to use ATN for discussion without becoming ATN developers.
...
When 주군 or another user has already asked for the cross-team ATN discussion and the minimal start gate passes, do not ask for another approval; start the council. `ready_to_start` means the moderator should proceed to `council.new`; `ready_for_approval` is not the live-visible discussion start signal.
...
Keep ATN development authority separate from ATN usage: cross-team KLM members may use ATN as discussion participants without becoming owners of `atn-control` or this plugin repository.
