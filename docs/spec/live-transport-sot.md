...
- For Discord-origin decision-bearing councils after the default-setting update, the standard operator path is `standard_live_visible_decision_council`: live-visible thread binding, bounded `max_discussion_turns`, 120-second selected-runner timeout by default, relevance selected-speaker lifecycle, selected-runner linked speech, visible delivery proof, participant closeouts, proposal/vote, and final visible closeout/finalize evidence. This is still a daemon/control lifecycle and evidence contract; the plugin does not own lifecycle state or Discord sending.
...
- For Discord-origin live-visible selected-runner councils, `plugin/NEWFIX-006` must require `dispatch_timeout_sec=120` or an explicitly approved alternative before start-capable status, consuming control evidence from `control/NEWFIX-005`. Missing timeout evidence is a `start_blocker`; post-run missing timeout/prompt continuity proof remains `final_acceptance_unproven` unless a stricter start gate already blocked the run.
...
| 5 | control | NEWFIX-005 | implementation_complete/review_pending | Control-owned live-visible selected-runner timeout policy/evidence requiring `dispatch_timeout_sec=120` or approved alternative for Discord live-visible councils. Plugin consumes the resulting evidence only after the control contract is review-closed. |
