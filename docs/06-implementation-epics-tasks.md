...
| NEWFIX-008 | Council command live submit representation/idempotency contract | planned | Plugin-owned follow-up to repair or clarify `atn_council_command(council.lock_agenda)` live `command.submit` representation for structured agenda payloads, preserving caller-supplied IDs while keeping daemon `command_id` authority explicit and rejecting hidden CLI fallback or local ID generation. |
