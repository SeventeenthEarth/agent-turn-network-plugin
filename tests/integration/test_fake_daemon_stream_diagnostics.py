...
from kkachi_agent_network_plugin.errors import DaemonCompatibilityError, DaemonProtocolError
...
def test_fake_daemon_diagnostics_decodes_and_redacts() -> None:
    def diagnostics(body: JsonObject | None) -> JsonObject:
        assert body == {"protocol_version": "kan-protocol-v1alpha0", "session_id": "sess-int-2"}
        return {
            "protocol_version": "kan-protocol-v1alpha0",
            "daemon_version": "0.0.0-fake",
            "live_readiness": False,
    ...
    assert transport.requests == [
        (
            OP_DIAGNOSTICS_READ,
            {"protocol_version": "kan-protocol-v1alpha0", "session_id": "sess-int-2"},
        )
    ]
