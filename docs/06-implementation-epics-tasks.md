...
| LTRAN-002 | Add explicit plugin live daemon transport | planned | First implementation task after `plugin/LTRAN-001`. Add an explicit configured Unix-socket daemon client for plugin tools while preserving no-config fail-closed behavior and rejecting unsafe/missing/malformed/incompatible daemon paths without CLI, localhost, Discord, Hermes, gateway, auth, token, or KAB fallback. |
