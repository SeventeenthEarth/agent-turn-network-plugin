...
`plugin/NEWFIX-006` is implementation_complete/review_pending for the plugin guardrail consuming those control evidence contracts. The current plugin row remains a local planner/operator hardening proof pending focused re-review and does not authorize live Discord rollout, daemon startup, runtime/profile/provider/gateway/auth/token/model mutation, push, or production readiness.
