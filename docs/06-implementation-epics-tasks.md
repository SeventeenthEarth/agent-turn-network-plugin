...
| RUNFIX3-003 | Planner and evidence schema for live-thread proof axes | planned | Extend plugin planner/schema/operator evidence against the frozen control evidence contract so activation and final reports carry origin binding equality, expected/actual visible turns, participant closeout and moderator synthesis evidence, per-turn delivery target match, content/audit prompt envelope, dialogue-mode diagnostics, drift diagnostics, and negative fail-closed reporting without promoting live readiness. Fixed minimum gates: `git diff --check`, `make docs-guardrails`, `make check-core-contract`, `make test-prepare`, `make test-unit` unless explicitly waived, focused `uv run python -m pytest tests/unit/test_discussion_activation_planner.py tests/unit/test_tool_schemas.py tests/unit/test_tool_handlers.py tests/unit/test_core_contract.py tests/unit/test_diagnostics_decoding.py tests/unit/test_surface_rendering.py`, positive/negative sample planner or final-report payload, Red/Orange/Gray review, and Blue synthesis. |
