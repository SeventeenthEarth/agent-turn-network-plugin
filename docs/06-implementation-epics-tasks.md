...
| RUNFIX-015 | Visible author guard | planned | Implement/lock visible author preflight and reporting requirements for live-visible councils. Same posting path probes must record `profile`, expected author source (`registry_snapshot` or explicit approved profile-author map), `bot_id`, `username`, `source_env`, and `posting_path`, prove env precedence, fail closed before `council.new` on shared/default or unexpected authors, and record per-turn Discord message id/member/profile-author/speech linkage evidence. |
| RUNFIX-017 | ARGUE quality-required prompt contract | planned | Strengthen participant prompts and selected-response guidance for quality-required councils: include compact prior claim graph targets, require non-opening turns to carry valid `stance_links[]` or justified `new_axis`, preserve ARGUE fields through `kan_selected_participant_response`, fail `discussion_quality_pass` on the first orphan non-opening speech in `quality_required`, and report lifecycle-vs-quality counts without synthetic relation inference. |
