...
| NEWFIX-009 | Live-visible selected-runner agenda-context acceptance proof | planned | Cross-repo acceptance proof after `NEWFIX-007` and `NEWFIX-008`: verify the agenda payload and selected-runner prompt evidence include `decision_question`, `success_criteria`, and `out_of_scope_policy`, and verify missing fields still fail closed. The cancelled `sess_council_1782671073534714000` is diagnostic defect evidence only. |
