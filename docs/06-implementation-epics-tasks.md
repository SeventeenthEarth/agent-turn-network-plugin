...
Dependency gate: RUNFIX uses one global cross-repo task sequence. `plugin/RUNFIX-002` follows `control/RUNFIX-001`; `plugin/RUNFIX-006` follows `control/RUNFIX-005`; `control/RUNFIX-009` follows `plugin/RUNFIX-008`; `plugin/RUNFIX-010` follows `control/RUNFIX-009`.
...
| RUNFIX-006 | KAN discussion activation planner/doctor | planned | Add an approval-gated activation planner/doctor that checks plugin install/enabled state, explicit control daemon compatibility, participant profile tool visibility, selected Discord parent channel, profile eligibility, planned config changes, rollback, and blockers without hidden CLI fallback or live-readiness overclaim. Requires control/RUNFIX-005. |
| RUNFIX-007 | Discord eligibility and bot-to-bot exclusion | planned | Determine effective profile eligibility for KAN discussion allow-lists, exclude bot-to-bot enabled profiles by default, report exclusions and remediation, and prove parent-channel/thread allow-list behavior or fail closed with a Hermes gateway dependency. Requires plugin/RUNFIX-006. |
| RUNFIX-008 | Participant ARGUE response guidance and fallback reporting | planned | Update packaged operator guidance, participant response templates, and fallback disclosure reports so `claims[]`, `stance_links[]`, `contribution_type`, `new_axis_reason`, ARGUE counts, runner evidence, and canonical speech linkage are visible in operator evidence. Requires plugin/RUNFIX-007. |
| RUNFIX-010 | Live-local activation pilot and final operator package | planned | With explicit approval, run a visible live-local pilot using eligible profiles only, prove no-restart thread behavior, selected-speaker runner path, canonical speech linkage, ARGUE quality verdicts, and publish final readiness or exact blockers. Requires control/RUNFIX-009. |
