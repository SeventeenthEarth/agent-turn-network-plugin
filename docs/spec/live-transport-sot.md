...
The control-side companion SOT is `../../agent-turn-network-control/docs/spec/live-transport-control-sot.md`. This plugin SOT may describe daemon/CLI/member-runtime responsibilities for boundary clarity, but control-owned behavior is implemented and roadmapped in the control repository.
...
RUNFIX update: this SOT also records the plugin-owned side of `RUNFIX`, the cross-repo remediation epic created from the 2026-06-17 council dogfood issues report. `control/RUNFIX-001` and `plugin/RUNFIX-002` are accepted docs-only SOT locks after historical Red/Orange/Gray review, focused re-check, and Blue final synthesis; they do not install or activate live ATN discussion by themselves. RUNFIX3 update: this SOT also records the plugin-owned follow-up from the 2026-06-25 KLM live-thread postmortem; `RUNFIX3-001` is a docs-only SOT/roadmap lock and does not authorize live Discord rollout, profile/provider/gateway/auth/token/model mutation, daemon startup, push, or production readiness. NEXFIX update: this SOT records the plugin-owned guidance follow-up for the 2026-06-26 selected-runner prompt envelope defect and the 2026-06-28 live-visible runtime delivery follow-up. `control/NEWFIX-001` and `plugin/NEWFIX-002` are completed after focused color review, traceability repair, and Blue synthesis; `cross-repo/NEWFIX-003` is the completed docs-only SOT/diagnosis lock; `control/NEWFIX-004` is implementation_complete/review_pending for selected-member own-history prompt context, and `control/NEWFIX-005` is implementation_complete/review_pending for live-visible timeout policy/evidence.
`plugin/NEWFIX-006` is implementation_complete/review_pending for the plugin guardrail consuming those control evidence contracts. The current plugin row remains a local planner/operator hardening proof pending focused re-review and does not authorize daemon startup, runtime/profile/provider/gateway/auth/token/model mutation, push, or production readiness by itself. Default-setting update: Jooyoo's live-success report `17thHermes:40_outputs/team/jooyoo/atn/2026-07-03-atn-selected-speaker-vote-final-closeout-success-report.md` is now the concrete evidence anchor for promoting Discord-origin decision-bearing councils to `standard_live_visible_decision_council` by default; pass labels remain evidence-derived and production/runtime mutation gates remain separate.
...
Plugin roadmap entries live in `docs/roadmap.md`. Control roadmap entries live in `../../agent-turn-network-control/docs/roadmap.md`. When a task ID is referenced outside its repo-local roadmap or SOT table, use repo-qualified notation such as `plugin/LTRAN-001` or `control/LTRAN-001` to avoid ambiguity.
...
- For Discord-origin decision-bearing councils after the default-setting update, the standard operator path is `standard_live_visible_decision_council`: live-visible thread binding, bounded `max_discussion_turns`, 150-second selected-runner timeout by default, relevance selected-speaker lifecycle, selected-runner linked speech, visible delivery proof, participant closeouts, proposal/vote, and final visible closeout/finalize evidence. This is still a daemon/control lifecycle and evidence contract; the plugin does not own lifecycle state or Discord sending.
...
- normative live-thread procedure ownership now lives in `src/atn_plugin/bundled_skills/atn-moderator/SKILL.md` and `docs/spec/skill-and-operator-guide.md`;
...
NEXFIX records the 2026-06-26 selected-runner prompt envelope defect where a selected participant received a runner prompt without the locked agenda or prior context even though lifecycle/runtime readiness passed. It also records the 2026-06-28 KLM live-visible selected-runner runtime delivery blocker where a repaired Discord-thread session still failed selected-runner completion because of dispatch timeout, visible delivery context deadline, and selected-member prompt-continuity gaps. The 2026-06-29 cancelled session `sess_council_1782671073534714000` adds agenda required-context submission defect evidence: the control CLI lacked a structured `success_criteria` path and the plugin live command path exposed a daemon `command_id` representation boundary. SOT locks: `17thHermes:40_outputs/team/macho/atn/2026-06-26-atn-selected-runner-prompt-envelope-nexfix-sot.md` and `17thHermes:40_outputs/team/macho/atn/2026-06-28-atn-live-visible-selected-runner-runtime-delivery-newfix003-sot.md`. The epic label `NEXFIX` and task prefix `NEWFIX-*` are an explicit 주군-requested exception. NEXFIX did not by itself authorize production activation, live Discord rollout, default Discord activation, gateway/auth/token/provider/profile/model mutation, daemon startup/discovery, package publication, push, or broad rollout. The later default-setting update consumes concrete Jooyoo live-success evidence for Discord-origin decision-bearing council defaults while preserving separate production/runtime mutation gates.

Plugin-owned contract impact:

- Runtime readiness, content-plane readiness, prompt-continuity readiness, and live-visible timeout readiness are separate operator gates. Subscriber/heartbeat/cursor/prepare/ready success does not prove selected-runner prompt adequacy, selected-member own-history continuity, selected-runner timeout sufficiency, or Discord visible delivery success.
- `atn-moderator` guidance must require `selected_runner_prompt_evidence` review before long or live-visible councils and must stop the run if a first speech says agenda/prior context is missing. `selected_runner_prompt_evidence` is the named handoff from `control/NEWFIX-001`; after `control/NEWFIX-004`, that handoff also includes distinguishable selected-member own-history source evidence. Acceptable evidence is a status/export field, deterministic fixture artifact, or equivalent non-secret prompt-capture proof with `session_id`, `speaker_selected_event_id`, `selected_member`, `turn`, agenda/prior-context source refs, `included_context[]`, `missing_required_context[]`, `prompt_context_sha256`, redacted prompt excerpt or fixture id, and `result`.
- For Discord-origin live-visible selected-runner councils, `plugin/NEWFIX-006` must require `dispatch_timeout_sec=150` or an explicitly approved alternative before start-capable status, consuming control evidence from `control/NEWFIX-005`. Missing timeout evidence is a `start_blocker`; post-run missing timeout/prompt continuity proof remains `final_acceptance_unproven` unless a stricter start gate already blocked the run.
...
- `atn-participant` guidance must not produce generic speech when required agenda context is absent; it should return a diagnostic/fail-closed response that still satisfies the current control selected-runner producer contract in `../../agent-turn-network-control/docs/spec/operations.md#runner-stdout-semantic-framing` and canonical `speech` event shape in `../../agent-turn-network-control/docs/spec/protocol-and-cli.md#surface-rendering-evidence-contract` until a richer diagnostic event type is approved. Minimal compatible shape: `{"type":"speech","payload":{"speech":"Cannot provide a substantive council turn because the selected-runner prompt omitted required agenda/context; moderator should intervene or cancel.","claims":[{"claim_id":"missing_context","summary":"Required agenda/context was absent from the selected-runner prompt.","kind":"open_question"}],"stance_links":[],"contribution_type":"question","new_axis_reason":null,"evidence":[{"kind":"diagnostic","code":"selected_runner_context_missing"}]}}`.
- Plugin guidance consumes daemon projection authority from `control/NEWFIX-001`, implementation-complete/review-pending `control/NEWFIX-004`, and implementation-complete/review-pending `control/NEWFIX-005` timeout extensions; plugin-side hints are optional diagnostics only and cannot replace control projection, control timeout evidence, agenda payload authority, daemon command-id authority, or selected-runner delivery proof.
- `plugin/NEWFIX-008` implements the live `command.submit` representation for structured `council.lock_agenda` payloads without hidden CLI fallback, local lifecycle state, local dedupe, or generated daemon IDs. Live Unix socket transport sends the exact council command with the original payload as daemon `params`, so `decision_question`, `success_criteria`, and `out_of_scope_policy` remain under daemon-owned `params.payload`. Public `idempotency_key` remains caller-supplied but is representable only as `payload.command_id` or `idem-{payload.command_id}`; any other value fails closed before socket connect with operator guidance.
- `cross-repo/NEWFIX-009` is completed for focused local proof that plugin and control paths preserve `decision_question`, `success_criteria`, and `out_of_scope_policy` through `agenda_locked.payload`, selected-runner prompt evidence, and plugin live `command.submit` ingress; post-feedback remediation adds empty/whitespace `success_criteria` fail-closed proof and restores run-root verification evidence; focused Orange/Gray re-review passed, all five MAR roles are covered, and Blue MAR disposition allowed KAH `mar_review` completion; post-MAR second-color review, final verification, and improve/lessons are complete; live Discord delivery and broad readiness remain unclaimed.
...
| 3 | cross-repo | NEWFIX-003 | completed | Docs-only SOT/diagnosis lock records the 2026-06-28 live-visible runtime delivery blocker, including the 30-second effective default when no session timeout is set, shared generation/delivery deadline, and prompt own-history gap. Plugin mirrors the SOT and does not claim runtime mutation or live readiness. |
| 4 | control | NEWFIX-004 | implementation_complete/review_pending | Control-owned selected-member own-history prompt context and prompt evidence source separation are implemented with focused re-review pending. Plugin consumes the resulting evidence only after the control contract is review-closed. |
| 5 | control | NEWFIX-005 | implementation_complete/review_pending | Control-owned live-visible selected-runner timeout policy/evidence requiring `dispatch_timeout_sec=150` or approved alternative for Discord live-visible councils. Plugin consumes the resulting evidence only after the control contract is review-closed. |
| 6 | plugin | NEWFIX-006 | implementation_complete/review_pending | Plugin-owned activation planner/operator guardrail consuming `NEWFIX-004` and `NEWFIX-005` is implemented with Discord-origin live-visible start gated on visible-surface readiness, daemon registry membership, prompt own-history proof, and timeout proof. Local-only/artifact/manual bridge paths remain non-equivalent fallback, focused plugin re-review remains pending, and the row does not claim runtime mutation or live readiness. |
| 7 | control | NEWFIX-007 | completed | Control-owned agenda required-context submission repair now requires structured `decision_question`, `success_criteria`, and `out_of_scope_policy` at the CLI and daemon/storage boundary, aligns CLI/spec/conformance fixtures, and makes agenda file input explicit/fail-closed. Plugin records dependency status only; request-feedback and handle-feedback are accepted, and control MAR completed after focused arch re-review plus Blue disposition allowed KAH `mar_review` completion; second-color review has also passed and final verification and improve/lessons are complete. Second-color review, final verification, and improve/lessons gates are complete in control. This row does not grant plugin daemon authority or claim control live readiness, commit, or push. |
| 8 | plugin | NEWFIX-008 | completed | Plugin-owned live Unix socket `command.submit` representation now preserves structured `council.lock_agenda` agenda fields under daemon-owned `params.payload`, keeps daemon `payload.command_id` authority explicit, and fails closed before socket connect when caller-supplied `idempotency_key` is not representable as `payload.command_id` or `idem-{payload.command_id}`. The prior MAR architecture/status-surface blocker is resolved after focused re-review `t_6747d956`; Blue MAR disposition accepted the role set and KAH `mar_review`, second-color, final verification, and improve phases are complete with evidence at `.kkachi/runs/run-20260629T093316Z-4576122d46c7/mar-blue-disposition.md`. This row does not claim live daemon activation, Discord delivery, runtime readiness, commit, push, production readiness, or NEWFIX-009 acceptance proof. |
...
LVCOR records Jooyoo's 2026-06-30 request-changes package for one-pass live-visible council operation. Plugin-owned work remains downstream of the control-owned lifecycle/schema/status fixes and must not invent daemon lifecycle authority. The control companion SOT is `../../agent-turn-network-control/docs/spec/live-transport-control-sot.md`.

Plugin-owned contract impact:

- `plugin/LVCOR-004` must consume control-accepted canonical lifecycle fields. The canonical turn field is `payload.turn`; legacy `payload.round` is diagnostic/migration input only, must not be emitted by plugin golden envelopes, and is rejected locally on caller-supplied turn-bearing `atn_council_command` payloads before submit. Turn-bearing `council.poll`/`council.hand_raise`/`council.grant`/`council.speak` payloads also require a non-empty string or integer `payload.turn` before transport.
...
- `plugin/LVCOR-005` acceptance proof must consume completed control/plugin rows before any one-pass success claim, require both parameterized finalized-success rows `15/4/21` and `5/2/9`, treat `T20` only as the `15 + 4 + 1` instance, keep zero runnerless/manual selected turns for success-like rows, and preserve `unresolved_terminal_blocked` as blocker-path evidence rather than one-pass success. Live Discord delivery, daemon/profile/provider/gateway/auth/token/model mutation, production readiness, push, and broad rollout remain outside scope unless separately approved.
...
| 4 | plugin | LVCOR-004 | completed/local-proof | Plugin-owned lifecycle envelope golden tests and schema reconciliation are accepted for bounded local proof in run `run-20260702T141926Z-a716fc43a7d2`: canonical turn-bearing golden envelopes preserve `payload.turn` without `payload.round`; caller-supplied legacy nested `payload.payload.round`, missing `payload.turn`, and invalid turn values fail closed before transport; caller-supplied `command_id`/`request_id`/`idempotency_key` are preserved; and no plugin-local dedupe or lifecycle state is introduced. Official implementation review, focused Red/Gray re-review, feedback handling, MAR strict CVE/security reruns, post-MAR second-color review, and final KAH closeout are recorded in the run artifacts. This row does not approve live daemon activation, Discord delivery, profile/provider/gateway/auth/token/model mutation, commit, push, release, production readiness, or broad rollout. |
| 5 | cross-repo | LVCOR-005 | completed/local-kah-final | Plugin run `run-20260702T195344Z-341cf06c02d6` has local implementation and verification evidence for the explicit full-shape proof surface, including completed LVCOR-001 through LVCOR-004 dependency rows, finalized-success `15/4/21` and `5/2/9` scenarios, exact visible-turn count equality, T(n+p+1) terminal synthesis, zero runnerless/manual selected turns, participant closeout coverage, and finalized-vs-unresolved label separation. Focused status-remediation re-review accepted the original stale-row blocker, strict MAR rerun passed with all required roles covered, KAH `mar-review` completed at `evt-003295`, and post-MAR second-color review completed at `evt-003302` with Blue `APPROVE_WITH_FINDINGS / ATN_BLUE_SECOND_COLOR_ACCEPTED_WITH_FINAL_STATUS_RECONCILIATION_REQUIRED`. Final KAH closeout completed at `evt-003309`, workflow revision 41, with `final-report.md` and `gate-reports/final.json`. No live Discord delivery, runtime activation, production readiness, profile/provider/gateway/auth/token/model mutation, commit, push, release, or broad rollout is claimed. |


## PRSLR persistent participant runtime boundary

PRSLR is the fourteen-task cross-repo/direct epic for the persistent participant runtime SOT, the follow-on dialogue-quality hardening planned from the 2026-07-05 KLM token/speed final report (`17thHermes:40_outputs/team/jooyoo/atn/klm-token-speed-20260705T174553Z/final_report.md`), and the direct post-live-audit hardening follow-ups requested after the 2026-07-07 live KLM audits. PRSLR-012 is control-owned visible-delivery echo de-duplication; PRSLR-013 adds selected-runner claim-kind schema/prompt guidance, visible echo linkage guidance, and closeout lifecycle completion guidance across control/plugin surfaces. PRSLR-014 raises Discord live-visible selected-runner timeout evidence to 150 seconds, suppresses runnerless visible-echo re-recording after posted selected-runner canonical delivery at the control boundary, and mirrors the timeout/closeout guidance in plugin operator surfaces. The plugin side is a consumer/adapter surface only. Control owns participant-session handles, generations, response windows, cursor/ack authority, rehydrate snapshots, fail-close events, selected-runner accounting, discussion-quality diagnostics, and the named handoffs `persistent_participant_runtime_evidence` plus `prior_speaker_argue_quality_evidence` introduced by PRSLR-009.

Plugin PRSLR work may add `council.drop` schema/tool coverage and planner/operator reporting that consumes explicit control evidence. It must not create local lifecycle state, infer cursor coverage from Discord/Hermes messages, synthesize drops from observe failures, synthesize daemon-owned `auto=true` timeout drops, reintroduce stateless fallback, or claim live readiness from fake/injected transport tests. PRSLR-006 local proof exposes `council.drop` through `atn_council_command` only as an exact daemon-owned command envelope: caller `request_id` and `idempotency_key` are preserved, top-level `session_id` is written into the payload, `payload.payload.turn` is required, legacy `payload.payload.round` is rejected, manual drops require `request_event_id` and `reason`, and caller/plugin boolean `auto=true` fails closed before transport. PRSLR-006 implementation review, request_feedback, and handle_feedback are complete; `.gjc/` is ignored as transient local GJC diagnostic state, canonical GJC evidence remains under `.kkachi/runs/run-20260705T094456Z-e71c7b7d9795/artifacts/gjc/`. The first MAR provider run was held with `DEGRADED / unresolved_required_role_coverage`; Red adjudication `t_06900c3c` and Blue synthesis `t_3933d063` authorized a strict unresolved-role rerun for `logic`, `security`, and `test_adequacy`; the strict rerun passed with all required roles covered, non-blocking carry-forward findings recorded for PRSLR-007 hardening, and KAH `mar_review` completed at `evt-003350` after run-local state repair `evt-003349`. Official post-MAR second-color review completed at `evt-003354` with Red `t_84bd14ba`, Orange `t_c01cc684`, Gray `t_12e94e1a`, and Blue synthesis `t_a78ee68b` accepting final_verify routing with carry-forwards; KAH `final_verify` completed at `evt-003356`, KAH `improve` completed at `evt-003358`, and the plugin row is local-proof commit-ready. Fake/injected plugin fixtures are local proof only; PRSLR-007 persistent-runtime acceptance required control-produced cross-repo evidence. `PRSLR-007` classification is complete in primary control KAH run `run-20260705T132654Z-ae2091713390`; plugin remains the secondary transport/schema/operator proof surface, and its own evidence must consume control-produced fixtures or exports rather than acting as final authority. Final PRSLR dialogue-quality acceptance remains planned for `PRSLR-011` after `PRSLR-008` through `PRSLR-010`.

PRSLR-007 completed local/default-off proof in primary control KAH run `run-20260705T132654Z-ae2091713390` after ask-gated plan-vet, official implementation review, handle-feedback, accepted MAR, accepted post-MAR second-color, final verification, and improve closeout. The control run recorded explicit 주군 ask approval, started KAH-wrapped GJC ULTRAGOAL, produced local/default-off acceptance artifacts (`prslr007-acceptance-matrix.json`, `persistent-participant-runtime-evidence.json`, `plugin-consumption-proof.json`, `operator-report.md`, and manifest), and passed focused plus full control/plugin verification. The plugin proof remains consumption-only: fake/injected plugin tests are diagnostic rather than final authority, and plugin does not own daemon lifecycle, cursor, response-window, dedupe, recovery, or fallback state. Official implementation request-feedback and handle-feedback are complete; MAR provider review passed and Blue MAR disposition accepted post-MAR routing. Post-MAR second-color passed after focused Gray remediation and Blue synthesis; control KAH `second_color_review` completed at `evt-003903`, control KAH `final_verify` completed at `evt-003905`, and control KAH `improve` completed at `evt-003907`, workflow revision 27. Commit/push, plugin runtime activation, live/runtime readiness, Discord delivery, production readiness, provider/profile/gateway/auth/token/model mutation, and post-PRSLR activation remain unclaimed.

PRSLR-008 through PRSLR-011 are the dialogue-quality extension that preceded the later direct PRSLR-012/PRSLR-013/PRSLR-014 live-audit hardening follow-ups. `control/PRSLR-008` has completed bounded local proof and approved local commit for selected-runner invocation and canonical speech linkage: fallback/manual profile text, runnerless speech, wrong-turn speech, and missing-selected-turn speech cannot satisfy `selected_runner_pass`; control now requires runner invocation success plus linked canonical speech for the same selected member and positive selected turn. `control/PRSLR-009` has completed bounded local proof under control KAH run `run-20260706T092628Z-8b113d0833db`: classification, CodeGraph refresh, KAH-wrapped GJC RALPLAN receipt evidence, official plan-vet, ask approval, KAH-wrapped GJC ULTRAGOAL setup, implementation, enhance-test, optimize, docs update, official implementation review, no-source-change handle-feedback, MAR provider/script review, post-MAR second-color review, final verification, and improve closeout are recorded; current local control tests prove `prior_speaker_argue_quality_evidence`, fail-closed non-opening orphan rejection, display-only treatment of `responds_to_event_id`, valid prior speech/claim target accounting, and justified `new_axis` exception accounting. Control MAR merge-pack status is `PASS` with all required roles covered, `unresolved_required_roles=[]`, no Red adjudication trigger, watcher retirement evidence, and Blue disposition accepted for second-color; KAH `mar_review` completed at `evt-003995`. Initial post-MAR Orange/Gray status-surface blockers were remediated through focused re-review and Blue synthesis; KAH `second_color_review` completed at `evt-004002`. Final verification passed local control/plugin checks and completed at `evt-004007`; KAH `improve` completed at `evt-004009`, workflow revision `27`. `plugin/PRSLR-010` is completed/local-proof-committed local proof under plugin KAH run `run-20260706T120949Z-6faa592a3ca8`: after task classification, CodeGraph refresh, KAH-wrapped GJC RALPLAN receipt, KAH `plan`, official Red/Orange/Gray plan-vet, Blue synthesis `ATN_BLUE_PRSLR010_PLAN_VET_ACCEPTED_WITH_CARRY_FORWARDS`, explicit ask approval, KAH-wrapped GJC ULTRAGOAL setup, implementation, feedback handling, strict MAR rerun, and Blue MAR disposition, the plugin hardened schema/guidance/tests so non-opening selected responses in `quality_required` with sufficient caller context require caller-target-valid `participant_response.stance_links[]` or a justified `new_axis` exception. Local proof passed focused selected-response/schema tests plus `git diff --check`, `make docs-guardrails`, `make check-core-contract`, `make test-prepare`, and `make test`; KAH `second_color_review` completed at `evt-003397`, `final_verify` completed at `evt-003399`, `improve` completed at `evt-003401`, and PRSLR-010 local proof is completed/local-proof-committed via plugin commit `ce988e2` and control mirror commit `fc673b4`. The plugin still does not own lifecycle, event log, cursor/window, dedupe, selected-runner pass labels, or discussion-quality pass labels, and display/prose hints remain non-authority. `cross-repo/PRSLR-011` proves the E2E dialogue-quality path and final docs closeout while preserving the same no-live/no-production/no-mutation boundaries. PRSLR-011 has advanced beyond plan-vet in primary control KAH run `run-20260706T144956Z-c4f49751f7f4`: plan-vet accepted after Gray remediation, ask/implementation/enhance-test/optimize/docs/request-feedback/handle-feedback/MAR are complete through `evt-004052`, official post-MAR `second_color_review` started at `evt-004053`, and `second_color_review` completed at `evt-004059`; it remains local/default-off and does not claim final verification completion, improve completion, push, live/runtime activation, Discord delivery, or production readiness.

PRSLR-011 control implementation proof is now in bounded local/default-off control-completed/local-proof-committed state under primary control KAH run `run-20260706T144956Z-c4f49751f7f4`: request_feedback completed at `evt-004045`, focused Orange remediation was accepted, handle_feedback completed at `evt-004047`, control MAR completed at `evt-004052`, control `second_color_review` started at `evt-004053`, and control `second_color_review` completed at `evt-004059` after Red `t_529c1666`, Orange `t_019580e4`, Gray `t_776e2b69`, focused Orange `t_230f3cb0`, focused Gray `t_102d90f9`, and dependent Blue `t_5564ae23`. Plugin evidence remains consumer-only: focused selected participant response, schema, and bundled-skill tests prove the plugin preserves valid `stance_links[]`/justified `new_axis` evidence and rejects orphan quality-required responses when caller context is sufficient, while the daemon/control harness remains the authority for selected-runner accounting, `prior_speaker_argue_quality_evidence`, lifecycle, cursor/window state, fallback diagnostics, and discussion-quality pass labels. This does not claim Discord delivery, live/runtime activation, production readiness, profile/provider/gateway/auth/token/model mutation, package publication, push, or broad rollout; control final verification completed at `evt-004062`, improve/status closeout completed at `evt-004064`, workflow revision `27`, and no ready nodes remain.
...
- `control/LTRAN-001` is completed in `../../agent-turn-network-control/docs/spec/live-transport-control-sot.md` and `../../agent-turn-network-control/docs/roadmap.md` as the control-side SOT/mapping task.
