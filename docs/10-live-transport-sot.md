...
| `kan_discussion_activation_plan` | build dry-run discussion activation planner/doctor report | activation planning evidence | pure/local RUNFIX-006 planner only; explicit caller-provided evidence, bot-to-bot exclusion, parent-channel inheritance proof/blocker, separated RUNFIX labels, `live_readiness` remains false |
