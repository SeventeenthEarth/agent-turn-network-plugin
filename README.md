...
`kkachi-agent-network-plugin` is the Python Hermes plugin adapter for KAN. In the current CNDIS-1 state it exposes fake/injected Hermes plugin tools only: read-only `kan_daemon_status`, `kan_compatibility_diagnostics`, `kan_stream_tail`, command-envelope `kan_delegate_new` and `kan_delegate_action`, plus `kan_council_command` and `kan_delivery_evidence`. Hermes has a plugin slash-command host API, but this plugin still has no KAN slash-command bindings, live daemon discovery, Discord helpers, session-status tool, or installed-plugin smoke claim.
...
CNDIS-1 fake/injected plugin stage. The package layout, plugin manifest, root Hermes directory entrypoint, status/diagnostics/stream-tail tools, delegation/review command-envelope tools, and council/delivery-evidence command tools are in place. Handlers return JSON strings, preserve fail-closed error categories, redact sensitive diagnostics/stream payloads, and require explicit fake/injected `DaemonClient` factories.
