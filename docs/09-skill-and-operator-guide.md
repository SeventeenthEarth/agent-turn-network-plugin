...
## RUNFIX post-pilot guardrails (RUNFIX-014/RUNFIX-015 proof, RUNFIX-016..017 planned)

주유's 2026-06-19 rename-council handoff is registered as `RUNFIX-014` through `RUNFIX-017`. `control/RUNFIX-014` and `plugin/RUNFIX-015` now have local implementation proof; until the remaining plugin/control tasks are implemented and reviewed, operators must treat the following as local proof or required planning/acceptance boundaries, not completed runtime capability:
