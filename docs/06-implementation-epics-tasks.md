...
Exit: long or live-visible selected-runner councils have separate runtime readiness, content-plane readiness, prompt-continuity readiness, and timeout readiness gates. Control owns the projection-backed prompt envelope in `control/NEWFIX-001`, the selected-member own-history prompt evidence in `control/NEWFIX-004`, and the live-visible timeout policy/evidence in `control/NEWFIX-005`; plugin owns packaged moderator/participant guidance in `plugin/NEWFIX-002` and activation-planner/operator guardrails in `plugin/NEWFIX-006`. NEXFIX does not authorize production activation, live Discord rollout, default Discord activation, daemon startup/discovery, gateway/auth/token/provider/profile/model mutation, KAB readiness, push, package publication, or broad rollout; any live pilot remains separately approved and outside these local NEWFIX tasks.

Dependency gate: `plugin/NEWFIX-002` consumes the completed `control/NEWFIX-001` contract without inventing daemon authority. `plugin/NEWFIX-006` must consume the stable `control/NEWFIX-004` and `control/NEWFIX-005` evidence contracts before final acceptance. The named handoff target is `selected_runner_prompt_evidence`: a control status/export field, deterministic fixture artifact, or equivalent non-secret prompt-capture proof that records selected member, turn, causation, agenda/prior-context source refs, included/missing context keys, prompt hash or redacted excerpt, `result`, and after `NEWFIX-004` distinguishable selected-member own-history source ids. The plugin must not treat `council.grant` hints, visible messages, operator prose, or local-only artifacts as authoritative replacement for daemon projection or timeout evidence.
