...
9. [RUNFIX3-R09] `selected_runner_pass` remains an evidence-derived label and stays false when the selected runner fails. The moderator/operator must not continue a selected-runner turn by invoking profile chat wrappers, `/Users/draccoon/.local/bin/<profile> chat -Q -q`, `send --json`, `moderator_direct`, manual profile text, fallback profile text, or moderator reposting as substitute participant speech. If a fallback path is used for diagnostics, label it lifecycle/fallback evidence only, keep `selected_runner_pass=false`, and close unresolved or restart from a valid selected-runner path before claiming success.
...
For PRSLR-006 local plugin proof, manual `council.drop` is submitted through `atn_council_command` as an exact daemon command only when the caller supplies `payload.payload.turn`, `payload.payload.request_event_id`, and `payload.payload.reason`. The plugin preserves caller `request_id`, caller `idempotency_key`, and daemon command payload shape, but it does not decide response-window freshness, session ownership, cursor authority, or timeout state. Operators must omit `auto` for manual drops unless a later control-owned contract explicitly requires `auto=false` as non-authoritative caller metadata; `auto=true` is rejected by the plugin because timeout auto-drops are daemon-owned.
...
A PRSLR operator report is not complete unless it shows the control-produced `persistent_participant_runtime_evidence` source and the following minimum fields: readiness label, evidence paths, session reuse reference or redacted handle hash, mode, `participant_runtime_mode=persistent_delta`, `hot_prompt_strategy=delta_only`, omitted-full-history proof, `stateless_fallback=false`, all-member cursor coverage, response-window close reason and elapsed time, manual drop versus daemon auto-drop accounting, late-ignore accounting, rehydrate status or fail-close event, next owner/gate, and non-claims. Fake/injected plugin fixtures are local proof only until PRSLR-007 cross-repo acceptance consumes control-produced evidence; plugin planner inputs may test rendering only, not runtime continuity.

## Selected-runner fallback prohibition
